...
def get_solution(player_mode, solution_size):
    ...
    if player_mode == '2':
        solution = input("MasterMind, please enter a solution of {} colors: ".format(solution_size)).split()

        # Check length
        if len(solution) != solution_size:
            print('Your input is invalid. Please enter a solution of {} colors.\n'.format(solution_size))
            get_solution('2')
        ...
        return randomizer(possible_inputs, solution_size)
...
def mastermind():
    ...
    sleep(2.5)
    screen_clear()
    solution = get_solution(mode, SOLUTION_SIZE)
